...
internal static class CompatibilityHarnessApplication
{
    ...
    static async Task WriteContractBatchAsync(
        IPantsDatabase database,
        ...
        CancellationToken cancellationToken)
    {
        await using var transaction = await database.BeginTransactionAsync(
            ...
            cancellationToken).ConfigureAwait(false);
        transaction.Put(Encoding.UTF8.GetBytes(keyPrefix), value);
        transaction.Insert(Encoding.UTF8.GetBytes($"{keyPrefix}/insert"), value);
        transaction.Put(
            Encoding.UTF8.GetBytes($"{keyPrefix}/ttl"),
            value,
            TimeSpan.FromDays(365));
        transaction.Delete(Encoding.UTF8.GetBytes($"{keyPrefix}/deleted"));
        transaction.Put(Encoding.UTF8.GetBytes($"{keyPrefix}/range/m"), value);
        transaction.DeleteRange(
            Encoding.UTF8.GetBytes($"{keyPrefix}/range/a"),
            Encoding.UTF8.GetBytes($"{keyPrefix}/range/z"));
        ...
        CancellationToken cancellationToken)
    {
        var actual = await transaction.GetAsync(
            Encoding.UTF8.GetBytes(keyText),
            cancellationToken).ConfigureAwait(false);
        if (actual is null || !actual.Value.Span.SequenceEqual(expected.Span))
        {
            throw new InvalidOperationException(
                $"Unexpected value for key '{keyText}' in column family '{columnFamily.Name}'.");
        }
    ...
    static async Task AssertAbsentAsync(
        IPantsTransaction transaction,
        string keyText,
        IPantsColumnFamily columnFamily,
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Expected key '{keyText}' to be absent in column family '{columnFamily.Name}'.");
        }
    }
