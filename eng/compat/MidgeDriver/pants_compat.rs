...
fn put_producer(
    ...
    let key_prefix = format!("{prefix}/{producer}");
    let value = format!("created-by-{producer}").into_bytes();
    transaction.put(key_prefix.clone().into_bytes(), value.clone(), None)?;
    transaction.insert(
        format!("{key_prefix}/insert").into_bytes(),
        value.clone(),
        None,
    )?;
    transaction.put(
        format!("{key_prefix}/ttl").into_bytes(),
        value.clone(),
        Some(4_102_444_800_000),
    )?;
    transaction.delete(format!("{key_prefix}/deleted").into_bytes())?;
    transaction.put(format!("{key_prefix}/range/m").into_bytes(), value, None)?;
    transaction.delete_range(
        format!("{key_prefix}/range/a").into_bytes(),
        format!("{key_prefix}/range/z").into_bytes(),
    )?;
...
fn assert_producer(
    ...
    for suffix in ["insert", "ttl"] {
        let variant_key = format!("{key}/{suffix}");
        let actual = transaction
            .get(variant_key.as_bytes())?
            .ok_or_else(|| format!("missing compatibility key '{variant_key}'"))?;
        if actual.as_ref() != expected {
            return Err(format!("compatibility value for '{variant_key}' did not match").into());
        }
    }
    for suffix in ["deleted", "range/m"] {
        let variant_key = format!("{key}/{suffix}");
        if transaction.get(variant_key.as_bytes())?.is_some() {
            return Err(format!("compatibility key '{variant_key}' should be absent").into());
        }
    }
