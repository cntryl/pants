...
{
    readonly List<TrieNode> _nodes = [new(0, [], null)];
    byte[] _lastKey = [];
    ...
    public void Add(ReadOnlySpan<byte> key, uint blockId)
    {
        ...
        {
            throw new StorageException("Trie block keys must be added in sorted order.");
        }

        if (blockId == uint.MaxValue)
        {
            throw new StorageException("The maximum trie block ID is reserved.");
        }
    ...
    public IReadOnlyList<TrieNode> Finish() => _nodes;
    ...
    void Insert(ReadOnlySpan<byte> key, uint blockId)
    {
        ...
        while (matchedLength < key.Length)
        {
            ...
            if (!_nodes[currentIndex].TryGetChild(remaining[0], out var edge))
            {
                ...
                _nodes.Add(new TrieNode(prefixLength, remaining.ToArray(), blockId));
                _nodes[currentIndex].AddChild(new TrieEdge(remaining[0], newIndex));
                return;
            }
        ...
        uint blockId)
    {
        ...
        intermediate.AddChild(new TrieEdge(oldSuffix[0], oldRemainderIndex));
        ...
        {
            var newIndex = checked((uint)_nodes.Count);
            _nodes.Add(new TrieNode(splitPrefixLength, newSuffix, blockId));
            intermediate.AddChild(new TrieEdge(newSuffix[0], newIndex));
        }
