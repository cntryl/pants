    ...
    public static byte[] Decompress(ReadOnlySpan<byte> bytes, byte algorithm)
    {
        switch (algorithm)
        {
            ...
            case 1:
                if (bytes.Length < 4)
                {
                    throw new StorageException("Midge LZ4 payload has no size prefix.");
                }
                ...
                {
                    throw new StorageException("Midge LZ4 payload could not be decompressed.");
                }
