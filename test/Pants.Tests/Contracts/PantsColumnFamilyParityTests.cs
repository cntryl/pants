...
public sealed class PantsColumnFamilyParityTests
{
    ...
    public async Task ShouldValidateNamesAndAllocateMonotonicColumnFamilyIdentities()
    {
        ...
        var first = await database.CreateColumnFamilyAsync("first");
        var duplicate = await database.CreateColumnFamilyAsync("first");
        ...
        var recreated = await database.CreateColumnFamilyAsync("first");
        var maximum = await database.CreateColumnFamilyAsync(maximumName);
        ...
        await Assert.ThrowsAsync<PantsInvalidArgumentException>(() => database.DropColumnFamilyAsync(first).AsTask());
        await Assert.ThrowsAsync<PantsInvalidArgumentException>(() =>
    ...
    public async Task ShouldIsolateAndPersistColumnFamiliesAcrossFlushAndCompaction()
    {
        ...
        {
            var alpha = await database.CreateColumnFamilyAsync("alpha");
            var beta = await database.CreateColumnFamilyAsync("beta");
    ...
    public async Task ShouldDistinguishSafeAndDestructiveColumnFamilyDrop()
    {
        ...
        await using (var database = await PantsDatabase.OpenAsync(
                         PantsOpenOptions.Local(directory.Path)))
        {
            var safe = await database.CreateColumnFamilyAsync("safe");
            var destructive = await database.CreateColumnFamilyAsync("destructive");
    ...
    public async Task ShouldKeepInMemoryOperationsEphemeralAndIsolated()
    {
        await using (var database = await PantsDatabase.OpenAsync(PantsOpenOptions.InMemory()))
        {
            var family = await database.CreateColumnFamilyAsync("ephemeral");
            await using (var writes = await database.BeginTransactionAsync(
                             family,
                             PantsTransactionMode.ReadWrite))
            {
            ...
            await using (var mutations = await database.BeginTransactionAsync(
                             family,
                             PantsTransactionMode.ReadWrite))
            {
