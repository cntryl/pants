...
public sealed class BenchmarkReportingTests
{
    ...
    {
        Assert.Equal(151, BenchmarkInventory.DiscoverScenarioIds().Count);
        Assert.Equal(151, BenchmarkInventory.DiscoverScenarioIds().Distinct(StringComparer.Ordinal).Count());
    }
