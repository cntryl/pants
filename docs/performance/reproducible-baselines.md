...
This command fails unless discovery contains exactly 151 unique scenarios and every BenchmarkDotNet report succeeds. It writes source and machine metadata plus the scenario inventory beside the CSV reports. The `Dry` job is a cold lifecycle validation, not statistical performance evidence.
