...
public class StrictGroupCommitSystemBenchmarks : Tier4Benchmark
{
    ...
    public async Task<int> CompleteLocalSystemAsync()
    {
                         ...
                         PantsOpenOptions.Local(_path).WithMemtableLimits(128 * 1024)))
        {
            for (var wave = 0; wave < FlushWaves; wave++)
            {
                ...
                var tasks = Enumerable.Range(0, Writers)
                    .Select(writer => CommitWriterAsync(database, capturedWave, writer));
        ...
        var count = 0;
        {
            await using var transaction = await reopened.Transactions.BeginAsync(
        ...
        if (count != TotalTransactions)
        {
            throw new InvalidOperationException(
                $"Reopened database contained {count} records; expected {TotalTransactions}.");
        }
