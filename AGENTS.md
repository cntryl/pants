...
`Pants.slnx` contains the .NET 10 projects. All projects share the `Cntryl.Pants` root namespace. Interfaces and public contracts live in `src/Pants.Abstractions`, grouped by domain (`Cloud`, `Storage`, `Transactions`, `Observability`, `Scan`, `Time`, `Exceptions`). The engine and provider implementation lives in `src/Pants.Core`; each domain's implementation is internal and lives under that domain's own `Internal/` subfolder (e.g. `Cloud/Internal`, `Storage/Internal`). Idiomatic service registration belongs in `src/Pants.DependencyInjection`. Tests are in `test/Pants.Tests`, mirroring the same domain folders plus `Contracts/` (parity/contract tests), `Compatibility/` (Midge fixtures), and `Support/` (failpoint handlers and test doubles). Benchmarks are in `bench/Pants.Benches`, and documentation in `docs`. Keep Markdown under `docs/*`, except this root guide. Do not add CLI or standalone tooling projects without explicit agreement.
